...
export default function CustomCursor({
    ...
    const onMouseMove = React.useCallback(({ clientX, clientY }) => {
        setCoords({ x: clientX - 5, y: clientY - 5})
        cursorInnerRef.current.style.top = clientY - 5 + 'px'
        cursorInnerRef.current.style.left = clientX - 5 + 'px'
        endX.current = clientX - 5
        endY.current = clientY - 5
    }, [])
    ...
    React.useEffect(() => {
        const clickables = document.querySelectorAll(
            'a, input[type="submit"], input, textarea, label[for], select, button, .link, .button'
        )
