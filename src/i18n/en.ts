...
export const en: Copy = {
  ...
  skills: {
    ...
    levels: {
      ...
      vue: 'Production',
      nuxt: 'Production',
      angular: 'Worked in it professionally',
      spring: 'Working knowledge',
    ...
    why: {
      c: 'The quadcopter firmware, its remote controller and the robotic arm are all C and C++ on bare metal. Writing the attitude loop meant caring about how long each iteration takes, so this is where I learned to read what the compiler actually produces rather than trusting it.',
      ts: 'What I write most. This portfolio is TypeScript, including the WebGL engine behind it, where the types carry real weight — shader uniforms and buffer layouts are exactly the kind of thing a compiler should be checking.',
      ...
      dart: 'The language behind the Cbienlà mobile app, written and maintained daily since 2025. Sound null safety and the isolate model are the parts I have had to think hardest about.',
      ...
      java: 'Used through coursework and consulting rather than owned end to end. Comfortable reading and extending a Java codebase; I would want a ramp-up before designing one.',
      node: 'Backends, tooling and Cloud Functions. Most of what I have learned here is about the event loop — what blocks it, and what that does to latency under load.',
      react: 'The framework I have built the most in, across client work and my own. This site is React, and the sphere is deliberately kept outside it: the engine owns its own loop and never triggers a render, because 60fps and reconciliation do not mix.',
      ...
      angular: 'Worked in Angular codebases as a consultant. Dependency injection and RxJS were the adjustment; it is the framework on this list I would most want a refresher on.',
      spring: 'Built services with it in a JVM context. Solid on the basics — controllers, dependency injection, persistence — but I have not run one in production long enough to have opinions on tuning it.',
      'react-native': 'Mobile work before Flutter. It is why I understand what a cross-platform framework actually costs you at the native boundary, which is the thing that decides whether the choice was right.',
      flutter: 'The Cbienlà app. I have built and shipped its screens, state and platform integrations, and dealt with the awkward parts — deep links, background state, and the store review cycle.',
      aws: 'EC2, S3 and IAM from coursework and side projects. Enough to deploy and to reason about a bill; not enough to design an account structure for a team.',
